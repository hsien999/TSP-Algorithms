...
class TourConstructionHeuristics(BaseAlgorithm):
    ...
    def closest_neighbor(self, tour, node, in_tour=False, farthest=False):
        ...
        current_dist = [(c, neighbors[c]) for c in self.cities
    ...
    def nearest_insertion(self, farthest=False):
        ...
        tour_length = length
        while len(tour) != self.size:
            best, dist = None, 0 if farthest else float_('inf')
            ...
            for candidate in self.cities:
                ...
                _, length = self.closest_neighbor(tour, candidate, in_tour=True, farthest=farthest)
    ...
    # add k between the edge (i, j), resulting in a tour with sub tours (i, k, j)
    # used for the cheapest insertion algorithm
    def add_closest_to_tour(self, tour):
        best_dist, new_tour = float_('inf'), None
    ...
    def cheapest_insertion(self):
        # best_tour, best_length = None, float_('inf')
